...
class BlickfeldDataset(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        ...
        kitti_infos = [info for info in kitti_infos if info is not None]
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            pred_dict['alpha'] = -np.arctan2(-pred_boxes[:, 1], pred_boxes[:, 0]) + r.flatten()
            # pred_dict['bbox'] = pred_boxes_img
            pred_dict['dimensions'] = np.concatenate([l, h, w], axis=-1)
            # pred_dict['location'] = pred_boxes_camera[:, 0:3]
            # pred_dict['rotation_y'] = pred_boxes_camera[:, 6]
            pred_dict['rotation_y'] = r.flatten()
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=40):
    dataset = BlickfeldDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        create_kitti_infos(
            ...
            data_path=ROOT_DIR / 'data' / 'blickfeld',
            save_path=ROOT_DIR / 'data' / 'blickfeld'
        )
