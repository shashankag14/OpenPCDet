...
class RoIHeadTemplate(nn.Module):
    ...
    def pre_loss_filtering(self):
        ...
        rcnn_cls_labels_ul = rcnn_cls_labels[unlabeled_inds]
        rcnn_cls_preds = torch.sigmoid(rcnn_cls_preds)
        ...
        self.forward_ret_dict['reg_valid_mask'] = filtering_mask.long()

        # ----------- RCNN_CLS_LABELS -----------
        fg_mask = rcnn_cls_labels_ul > self.model_cfg.TARGET_CONFIG.UNLABELED_CLS_FG_THRESH
        bg_mask = rcnn_cls_labels_ul < self.model_cfg.TARGET_CONFIG.UNLABELED_CLS_BG_THRESH
        ignore_mask = torch.eq(self.forward_ret_dict['gt_of_rois'][unlabeled_inds], 0).all(dim=-1)
        rcnn_cls_labels_ul[fg_mask] = 1
        rcnn_cls_labels_ul[bg_mask] = 0
        rcnn_cls_labels_ul[ignore_mask] = -1
        self.forward_ret_dict['rcnn_cls_labels'][unlabeled_inds] = rcnn_cls_labels_ul
